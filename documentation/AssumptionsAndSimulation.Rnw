...
\\
\\
In the parameterization above there are ten free parameters.  All else held equal, fewer parametesr are better.  Note that both \(\delta\) and \(q\) have very similar impacts on the results.  Hence we set \(q=1\) and let \(\delta\) manage the size of the jumps.  We choose to let \(\delta\) be free since with a fixed \(\delta\) the model only converges to Heston's model for \(q \to \infty\).  
