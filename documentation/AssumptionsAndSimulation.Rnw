...
\[\beta'(t) = -\psi(u) +\left(a+\delta \lambda \mathbb{E}[z_j]-iu\rho\sigma\eta \right) \beta(t)-\frac{\beta^2(t)\eta}{2}- \int_\Omega \left(e^{(\beta(t)\delta+iu) z} -e^{iu z}\right) \lambda \mu(dz) \]
\[\alpha'(t) = - a \beta(t) \]
...
simulateExpJump=function(numJumps){
  if(numJumps>0){
    return(delta*rexp(numJumps, lambda))
  }
  else{
    return(0)
  }
...
simulateGaussJump=function(numJumps){
  if(numJumps>0){
    return(rnorm(numJumps, muL, sigL))
  }
  else{
    return(0)
  }
}
generateOptionPrice=function(m){
  v=c(1:m)
  s=c(1:m)
  v[1]=v0
  s[1]=s0
  w2=rnorm(m-1)
  w1=w2*rho+rnorm(m-1)*sqrt(1-rho*rho)
  for(j in c(2:m)){
    numJ=rpois(1, v[j-1]*lambda*dt)
    s[j]=s[j-1]*exp(r*dt-sig*sig*.5*v[j-1]*dt+lambda*v[j-1]*dt*(exp(muL+.5*sigL*sigL)-1)+sqrt(v[j-1]*dt)*sig*w1[j-1]+simulateGaussJump(numJ))
    v[j]=v[j-1]+a*(1-(delta*lambda/(a*q)+1)*v[j-1])*dt+eta*sqrt(abs(v[j-1])*dt)*w2[j-1]+simulateExpJump(numJ)
  }
  if(s[j]>k){
    return(s[j]-k)
  }
